...
\end{codetight}	
\begin{codetight}{Feature Module Weighted}
	refines class Edge { double weight; ... }
\end{codetight}	
\begin{codetight}{Feature Module ShortestPath}
	refines class Graph {
		List<Edge> shortestPath(Node a, Node b){
			List<Edge> currentPath;
			... 
			double currentLength = length(currentPath);
			... 
		}
		double length(List<Edge> path) {
			return path.stream()
...
\begin{frame}{Handling Feature Interactions: General Goals}
	\centering
	\pic[width=.58\linewidth]{interaction-handling-2_}
	\vskip15pt
	
	\pic[width=\linewidth]{interaction-handling-1_}
\end{frame}
...
\begin{frame}[fragile]{\myframetitle{}}
	\begin{fancycolumns}[animation=none]
		...
		\textcolor{lecturegreen}{$ShortestPath \pimplies Weighted$}
		
		~
		\begin{note}{}
			Same implementation as before, but we make implementation dependency explicit:\\
			$ShortestPath$ requires $Weighted$.
		\end{note}
		\begin{note}{}
			Effectively forbids faulty interaction\\
			$\neg Weighted \wedge ShortestPath$
		\end{note}
		\nextcolumn
\begin{codetight}{Feature Module BasicGraph}
	class Edge { Node a, b; ... }
...
\begin{frame}[fragile]{\myframetitle{} II}
	\begin{fancycolumns}[animation=none]
		\centering
		\featureDiagram{
			Graph,concrete
		...
		\textcolor{lecturegreen}{$\neg ShortestPath$}
		
		~
		\begin{note}{}
			Same implementation as before, but forbids using $ShortestPath$ in any configuration.
		\end{note}
		\nextcolumn
\begin{codetight}{Feature Module BasicGraph}
	class Edge { Node a, b; ... }
...
\begin{frame}[fragile]{\myframetitle{} III}
	\begin{fancycolumns}[animation=none]
		\centering
		\featureDiagram{
			Graph,concrete
		...
		\textcolor{lecturegreen}{$Weighted$}
		
		~
		\begin{note}{}
			Same implementation as before, but requires $Weighted$ in every configuration\\
			(i.e. makes $Weighted$ \textit{core}).
		\end{note}
		\nextcolumn
\begin{codetight}{Feature Module BasicGraph}
	class Edge { Node a, b; ... }
...
\subsubsection*{Add Domain Dependency}
\begin{frame}{\myframetitle}
	\begin{fancycolumns}[widths={30},animation=none]
		\begin{definition}{Strategy S1}
...
\begin{frame}[fragile]{\myframetitle}
	...
	\nextcolumn
\vspace{-10pt}
\begin{codetight}{Feature Module \emph{UnweightedShortestPath}}
	refines class Graph {
		List<Edge> shortestPath(Node a, Node b){
			List<Edge> currentPath;	...
			double length = length(currentPath);	... 
		}
		double length(List<Edge> path) {
			@return path.size();@
		}
...
\end{codetight}	
\begin{codetight}{Feature Module \emph{WeightedShortestPath}}
	refines class Graph {
		List<Edge> shortestPath(Node a, Node b){
			List<Edge> currentPath;	... 
			double length = length(currentPath);	... 
...
\subsection{Strategy S4: Move Coordination Code}
...
\begin{frame}[fragile]{\myframetitle{}}
	\begin{fancycolumns}[animation=none]
		...
		~
		\begin{note}{}
			Move relevant code to one of the features (or to a third one all interacting features depend on).
		\end{note}
		\nextcolumn
\begin{codetight}{Feature Module BasicGraph}
	class Edge { Node a, b; @double weight = 1.0;@ ... }
\end{codetight}	
\begin{codetight}{Feature Module Weighted}
	refines class Edge { ... }
\end{codetight}	
...
\subsection{Strategy S5: Conditional Coordination Code}


\subsubsection*{Derivative Modules}
...
\begin{frame}[label=HandlingFeatureInteractions]{\myframetitle}
	\centering
%	\pic[width=0.9\linewidth,page=8,trim=40 25 40 25,clip]{interaction-handling} % TODO animated table in LaTeX
	\pic[width=.9\linewidth]{interaction-handling-overview}
\end{frame}
