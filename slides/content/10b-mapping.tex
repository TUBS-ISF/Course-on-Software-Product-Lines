...
\begin{frame}{\myframetitle}
	\begin{fancycolumns}[widths={46,54}]
...
\begin{frame}[b,fragile]{\myframetitle}
	...
	\begin{fancycolumns}[t,columns=3,widths={40,23,37},animation=none]
		...
		\end{definition}
		\begin{note}{}
			calculated by querying a satisfiability solver whether $PC$ is \emph{not satisfiable}:\\
			$\pnot SAT(PC)$
		\end{note}
		\uncover<3->{
...
\begin{frame}[b,fragile]{\myframetitle}
	...
	\begin{fancycolumns}[t,columns=3,widths={40,23,37},animation=none]
		...
		\end{definition}
		\begin{note}{}
			calculated by querying a satisfiability solver whether $PC' \pand \pnot PC$ is \emph{not satisfiable}:\\
			$\phantom{\equiv{}} \pnot SAT(\pnot(PC' \mimplies PC))$\\
			$\equiv{} \pnot SAT(PC' \pand \pnot PC)$
		\end{note}
		\uncover<2->{
			\begin{example}{}
				\begin{itemize}
...
\begin{frame}[t,fragile]{\myframetitle}
...
class Edge {
	...
	\end{fancycolumns}
	\begin{alignat*}{3}
	PC_{location} \defeq &~\Phi(FM) \hskip5.5em \pand PC_{\texttt{edge.cpp}} \hskip5em \pand PC_{\texttt{pair<Node, Node> nodePair;}}\hskip9em
	\\[1.5ex]
	\uncover<2->{= &~G \pand N \pand E \pand \pnot (D \pand U) \pand (H \pimplies U) \pand (D \por H) \hskip3em \pand E \hskip3em \pand \pnot D \pand \pnot H \pand \pnot D}
	\\[1.5ex]
	\uncover<3->{\mimplies &~(D \por H) \pand \pnot D \pand \pnot H}
	\\
	\uncover<4->{\mimplies &~\bot \hskip2em {\color{lecturered}\rightarrow\text{code is dead!}}}
	\end{alignat*}
\end{frame}
...
\begin{frame}[t]{\myframetitle}
	\vspace*{-3ex}
	\begin{fancycolumns}[t,widths={34,66},animation=none]
		\begin{exampletight}{}
