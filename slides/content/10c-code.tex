...
\begin{frame}[t]{\myframetitle}
	\begin{fancycolumns}[t]
		\begin{example}{Reachability}
			For any identifier that is used (e.g., variable, field, method), there is a definition of it.
		\end{example}
		\begin{definition}{Reachability Condition of Identifier $id$}
			Let $id$ be some identifier. To guarantee that a given \emph{reference} to $id$ is also \emph{defined} somewhere following condition must hold:\\[1ex]
			$\Phi(FM) \mimplies (PC_{ref}^{id} \pimplies \bigvee_{def} PC_{def}^{id})$\\[2ex]
			or, with a SAT solver:\\[1ex]
			\small
			$\pnot SAT(\Phi(FM) \pand PC_{ref} \pand \bigwedge_{def} \pnot PC_{def})$
		\end{definition}
		\nextcolumn{}
		\begin{example}{Conflict}
			For any identifier, there are no multiple conflicting definitions of it.
		\end{example}
		\begin{definition}{Conflict Condition of Definitions of Identifier $id$}
			Let $id$ be some identifier and $d_i$ its definitions.
			To guarantee that no definition of $id$ \emph{conflicts} with another the following condition must hold:\\[1ex]
			$\Phi(FM) \mimplies \bigwedge_{d_1 \neq d_2} \pnot (PC_{d_1}^{id} \pand PC_{d_2}^{id}))$\\[2ex]
			or, with a SAT solver:\\[1ex]
			\small
			$\pnot SAT(\Phi(FM) \pand \bigvee_{d_1 \neq d_2} (PC_{d_1}^{id} \pand PC_{d_2}^{id}))$
		\end{definition}
	\end{fancycolumns}
\end{frame}
...
\begin{frame}[t]{\myframetitle}
	\begin{fancycolumns}[t,widths={37,63},animation=none]
		\vspace*{-3ex}
		\begin{exampletight}{}
		...
		\nextcolumn{}	
		\uncover<2->{\picDark[width=\linewidth]{fop-csur-example-error-blind}}
	\end{fancycolumns}
\end{frame}
